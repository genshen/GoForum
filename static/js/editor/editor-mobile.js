...
window.___E_mod(function (E, $) {
    ...
    E.config = {
        ...
        //渲染时,使用domai代替imgUrlTag;存储时使用imgUrlTag代替domain
        imgUrlTag: '#BASE_URL#',
                ...
                $inputFlie.on('change', function (e) {
                    ...
                    reader.onload = function (e) {
                        ...
                        xhr.onload = function () {
                            ...
                            var resultSrc = JSON.parse(xhr.responseText);
                            var $prevImgContainer = $('#' + prveImgContainerId);
                            var loadImg;
                            var $loadImg;
                            ...
                            // 返回数据错误
                            if (xhr.status != 200 || resultSrc.error) {  // resultSrc.error may equals undefined

                                log('很遗憾，后台返回error，错误信息为：' + resultSrc.error);
                                // 提示错误
                                alert('上传图片错误: \n' + resultSrc.error);
                                // 移除预览图片
                                $prevImgContainer.remove();
                                ...
                                loadImg.src = config.imgUploadConfig.domain + resultSrc.key;
                        ...
                        formData.append('token', config.imgUploadConfig.upload_token);
                        xhr.send(formData);
